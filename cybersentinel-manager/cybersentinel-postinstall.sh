...
#          for the CyberSentinel Manager deployment.
#
#          Host bind mounts (/var/ossec/etc, /var/ossec/integrations, /var/ossec/logs)
#          are written directly on the host. Container-only paths (/var/ossec/ruleset/)
#          are deployed via docker cp.
#
# Requirements:
...
GITHUB_TOKEN="${GITHUB_TOKEN:-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}"
...
validate_environment() {
    ...
    # Validate host bind mount directories exist
    local bind_dirs=("/var/ossec/etc" "/var/ossec/integrations" "/var/ossec/logs")
    for dir in "${bind_dirs[@]}"; do
        if [ -d "$dir" ]; then
            log_success "Host bind mount: $dir"
        else
            log_warn "Host bind mount missing: $dir — files will be deployed via docker cp"
        fi
    done
...
deploy_configurations() {
    log_info "Deploying configurations (host bind mount: /var/ossec/etc)..."
    ...
    if [ -f "$TEMP_DIR/config/ossec.conf" ]; then
        ...
        cp "$TEMP_DIR/config/ossec.conf" /var/ossec/etc/ossec.conf
...
deploy_rules() {
    ...
    # Deploy to /var/ossec/etc/rules/ (host bind mount)
    mkdir -p /var/ossec/etc/rules
    for rule_file in "$TEMP_DIR/rules"/*.xml; do
        if [ -f "$rule_file" ]; then
            local filename=$(basename "$rule_file")
            cp "$rule_file" "/var/ossec/etc/rules/$filename"
            log_info "  → Deployed $filename to /var/ossec/etc/rules/ (host)"
        fi
    done

    # Deploy to /var/ossec/ruleset/rules/ (container — named volume, persists across recreates)
    local ruleset_rules=(
        "0016-wazuh_rules.xml"
        "0015-ossec_rules.xml"
        "0490-virustotal_rules.xml"
    ...
    for rule in "${ruleset_rules[@]}"; do
        if [ -f "$TEMP_DIR/rules/$rule" ]; then
            ...
            log_info "  → Deployed $rule to /var/ossec/ruleset/rules/ (container)"
        fi
    done
    ...
    if [ -f "$TEMP_DIR/rules/0475-IDS_IPS_rules.xml" ]; then
        ...
        log_info "  → Deployed 0475-IDS_IPS_rules.xml as 0475-suricata_rules.xml (container)"
    fi
...
deploy_decoders() {
    log_info "Deploying decoders..."

    # Deploy to /var/ossec/etc/decoders/ (host bind mount)
    ...
    for decoder_file in "$TEMP_DIR/decoders"/*.xml; do
        if [ -f "$decoder_file" ]; then
            ...
            if [ "$filename" != "0005-wazuh_decoders.xml" ]; then
                cp "$decoder_file" "/var/ossec/etc/decoders/$filename"
                log_info "  → Deployed $filename to /var/ossec/etc/decoders/ (host)"
            fi
        fi
    done

    # Deploy to /var/ossec/ruleset/decoders/ (container — named volume, persists across recreates)
    if [ -f "$TEMP_DIR/decoders/0005-wazuh_decoders.xml" ]; then
        ...
        log_info "  → Deployed 0005-wazuh_decoders.xml to /var/ossec/ruleset/decoders/ (container)"
    fi
...
deploy_integrations() {
    log_info "Deploying integration scripts (host bind mount: /var/ossec/integrations)..."
    ...
    for integration_file in "$TEMP_DIR/integrations"/*; do
        if [ -f "$integration_file" ]; then
            ...
            cp "$integration_file" "/var/ossec/integrations/$filename"
            log_info "  → Deployed $filename (host)"
        fi
    done
...
fix_permissions() {
    ...
    # Host bind mounts — use GID 999 (wazuh inside container)
    chown root:999 /var/ossec/etc/ossec.conf 2>/dev/null || true
    chown -R root:999 /var/ossec/etc/rules/ 2>/dev/null || true
    chown -R root:999 /var/ossec/etc/decoders/ 2>/dev/null || true
    chown -R root:999 /var/ossec/integrations/ 2>/dev/null || true

    chmod 660 /var/ossec/etc/ossec.conf 2>/dev/null || true
    chmod 660 /var/ossec/etc/rules/*.xml 2>/dev/null || true
    chmod 660 /var/ossec/etc/decoders/*.xml 2>/dev/null || true
    chmod 750 /var/ossec/integrations/* 2>/dev/null || true

    log_success "Host bind mount permissions fixed"

    # Container-only paths (named volumes) — use docker exec
    ...
    log_success "Container volume permissions fixed"
}
...
verify_deployment() {
    ...
    # Host bind mount files — check directly on host
    local host_files=(
    ...
    # Container-only files — check via docker exec
...
main() {
    ...
    # Phase 3: Deploy to host bind mounts + container
    log_info "========== Phase 3: Deploying Files =========="
    deploy_configurations
    deploy_rules
    deploy_decoders
    deploy_integrations
